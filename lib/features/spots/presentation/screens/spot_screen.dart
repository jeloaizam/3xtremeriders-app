...
class _SpotScreenBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        _MediaGallery(spotId: spotId, onBack: onBack, onComingSoon: onComingSoon),
        Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (detail.hazzards.isEmpty)
                ...
                for (final hazzard in detail.hazzards) _HazzardRow(hazzard: hazzard),
              const SizedBox(height: 14),
              _HazardRatingCard(
...
class _MediaGalleryState extends ConsumerState<_MediaGallery>
  ...
  Widget build(BuildContext context) {
    ...
    final colors = context.colors;
    final l10n = AppLocalizations.of(context);
    final photos = ref.watch(spotPhotosProvider(widget.spotId)).value ?? const [];
    final videos = ref.watch(spotVideosProvider(widget.spotId)).value ?? const [];
    final isVideo = _tab == _MediaTab.video;
    final urls = isVideo
        ? [for (final v in videos) v.url]
              ...
              child: urls.isEmpty
                  ? Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Icon(
                            isVideo ? Symbols.videocam_off : Symbols.photo_camera,
                            size: 40,
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 8),
              _MediaFilterButton(label: l10n.spotMediaMore, onTap: widget.onComingSoon),
              const SizedBox(width: 8),
...
class _MediaFilterButton extends StatelessWidget {
  const _MediaFilterButton({required this.label, this.active = false, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: active ? colors.colorAction : colors.hairline),
          borderRadius: BorderRadius.circular(context.spacing.radiusMd),
        ),
        child: Text(
          label,
      ...
      child: Stack(
        ...
        children: [
          ...
          Positioned(
            ...
            bottom: 6,
            child: Container(
              padding: EdgeInsets.symmetric(horizontal: big ? 9 : 5, vertical: big ? 4 : 2),
              decoration: BoxDecoration(
              ...
              child: Text(
                ...
                style: context.typography.tag.copyWith(fontSize: big ? 11 : 9),
              ),
            ),
          ),
        ],
              ...
              children: [
                Row(
                  children: [
                    for (var i = 1; i <= 5; i++)
                      Icon(
                        avg >= i - 0.5 && avg < i ? Symbols.star_half : Symbols.star,
                        fill: avg >= i - 0.5 ? 1 : 0,
                        size: 20,
                        color: avg >= i - 0.5
                            ? colors.colorAction
                  ...
                  children: [
                    for (var i = 1; i <= 5; i++)
                      Icon(
                        ...
                        fill: avg >= i ? 1 : 0,
                        size: 20,
                        color: avg >= i ? colors.colorWarn : colors.hairlineStrong,
                      ),
                  ],
                ),
                const SizedBox(height: 5),
        ...
        child: Row(
          children: [
            Container(
              ...
              alignment: Alignment.center,
              child: Icon(elementIcon(element.type), size: 20, color: colors.colorBrand),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(element.name, style: context.typography.title.copyWith(height: 1)),
                ...
                for (var i = 1; i <= 5; i++)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: i <= difficulty ? colors.colorBrand : colors.hairlineStrong,
                    ),
                  ),
              ],
            ),
...
class _HazzardRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (Color color, Color background, Color border) = severity <= 2
        ? (colors.colorRating, colors.tintRating, colors.colorRating.withValues(alpha: .28))
        : severity == 3
        ? (colors.colorWarn, colors.tintWarn, colors.colorWarn.withValues(alpha: .28))
        : (colors.colorDanger, colors.tintDanger, colors.colorDanger.withValues(alpha: .28));
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(hazzard.name, style: context.typography.title.copyWith(height: 1)),
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          AppAvatar(
            initial: author.nickname.isNotEmpty ? author.nickname[0].toUpperCase() : '?',
            size: AppAvatarSize.sm,
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Text(author.nickname, style: context.typography.title.copyWith(fontSize: 14)),
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 5),
                    Text('${comment.voteCount}', style: context.typography.meta),
                  ],
                ),
              ],
            ),
